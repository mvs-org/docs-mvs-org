...
account类主要通过以下字段记录账户相关关键信息。
 ...
 hd\_index ----账户地址索引，对应于钱包从主助记词导入账户页面中的地址个数。主要用于记录账户地址个数的。其值加1即为该账户目前已建立地址个数，该字段以数值的形式存储于钱包本地账户数据库表中。
...
account\_address类主要通过如下字段存储地址关键信息。
...
asset\_detail类主要通过如下字段存储地址关键信息。
